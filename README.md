...
针对 **Hyperliquid Testnet API** 的自动化测试框架，用来持续回归账户、订单、仓位等核心能力，并自动生成 Allure 报告发布到 Cloudflare Pages。
...
- **一键 CI & 报告**：GitHub Actions 运行测试，生成 Allure 报告并自动部署到 Cloudflare Pages，可直接通过固定域名访问。
- **安全的测试网回放**：默认连 Testnet，读写分离、只对读操作重试，写操作不重试避免重复下单。
- **良好的测试隔离**：每个用例后自动清理 open orders，避免状态脏数据影响后续测试。
...
- **本地运行 pytest**  
  ...
  ```
  > **安全提示**：`.env` 已在 `.gitignore` 中，绝不要把私钥提交到仓库。
